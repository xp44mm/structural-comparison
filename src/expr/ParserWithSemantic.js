...
import { Stack } from '../compiler/Stack'

const _rules_ = Symbol('rules')
const _actions_ = Symbol('actions')
const _kernelSymbols_ = Symbol('kernelSymbols')
const _semantics_ = Symbol('semantics')

const _getTag_ = Symbol('getTag')
const _getLexeme_ = Symbol('getLexeme')

export class ParserWithSemantic {
    constructor({
        rules,
        actions,
        kernelSymbols,
        ...
        this[_semantics_] = new Map(semantics)
    ...
    setGetLexeme(getLexeme) {
        this[_getLexeme_] = getLexeme
    }
    ...
    parse(iterator) {
        ...
        const loop = (element) => {
            ...
            if (action === 0)
                ...
                let state = action
                let tree =  this[_getLexeme_](element.value) // { token: element.value }
                trees.push(tree)
                states.push(state)
                ...
                let semantic = this[_semantics_].get(action)
                console.log(children)
                console.log(semantic)
                let result = semantic(children)
                console.log(result)
                trees.push(result)
            ...
            else throw new Error("ParserWithSemantic.parse error: never")
        }
